  ...
  div
    div(v-for="item in value")
      ...
      component(
        ...
        tm-icon-expanded(v-if="iconExpanded(item) && level < 1" style="--icon-color: var(--color-primary, blue)").item__icon.item__icon__expanded
        tm-icon-collapsed(v-if="iconCollapsed(item) && level < 1" style="--icon-color: rgba(0,0,0,0.18)").item__icon.item__icon__collapsed
        tm-icon-internal(v-else-if="!outboundLink(item.path) && level < 1 && !iconExpanded(item)").item__icon.item__icon__internal
