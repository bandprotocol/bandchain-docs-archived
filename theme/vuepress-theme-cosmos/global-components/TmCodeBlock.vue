...
.expand__item {
  ...
  box-shadow: 0px 16px 32px rgba(22, 25, 49, 0.08), 0px 8px 12px rgba(22, 25, 49, 0.06),
    0px 1px 0px rgba(22, 25, 49, 0.05);
  box-sizing: border-box;
}
...
export default {
  ...
  methods: {
    filename(url) {
      ...
      if (tokens.length > 4) {
        return [tokens[0], tokens[1], "...", tokens.slice(-2)[0], tokens.slice(-2)[1]].join(" / ");
      } else {
        return tokens.join(" / ");
      }
    },
